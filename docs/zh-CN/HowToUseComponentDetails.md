...
## 用户的自定义规则
如果应用中没有用户想要的组件规则，并且用户知道该组件的来源，那么用户可以在应用中添加自定义规则。默认用户的自定义规则保存在应用存储中，Blocker会在读取组件信息的时候同步更新。
如果用户想要分享规则给第三方，用户可以点击应用详情界面上的分享按钮，将规则的压缩包发给作者添加进Blocker规则中。对于高级用户来说，用户也可以在GitHub的[规则仓库]((https://github.com/lihenggui/blocker-general-rules))上直接提交Pull request来添加规则。
Blocker会优先展示用户添加的自定义规则，如果自定义规则不存在，那么会显示软件内置规则。
