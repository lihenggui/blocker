...
fun BackupSettings(
    ...
    Column(
        ...
        ItemHeader(
            title = stringResource(id = string.feature_search_backup),
            extraIconPadding = true,
        )
        BlockerSettingItem(
            icon = ImageVectorIcon(BlockerIcons.Folder),
            title = stringResource(id = string.feature_search_folder_to_save),
            summary = ruleBackupFolder.ifEmpty {
                stringResource(id = string.feature_search_directory_invalid_or_not_set)
            },
            onItemClick = {
                ...
                if (intent.resolveActivity(context.packageManager) != null) {
                    ...
                    Toast.makeText(context, string.feature_search_file_manager_required, Toast.LENGTH_LONG).show()
                }
            },
        )
        SwitchSettingItem(
            itemRes = string.feature_search_backup_system_apps,
            checked = backupSystemApps,
            onCheckedChange = onChangeBackupSystemApp,
        )
        SwitchSettingItem(
            itemRes = string.feature_search_restore_system_apps,
