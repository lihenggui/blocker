...
import com.merxury.blocker.feature.applist.impl.component.AppSortBottomSheetRoute
import com.merxury.blocker.feature.applist.impl.component.TopAppBarMoreMenu
...
fun AppListScreen(
    ...
    Column(
        ...
        BlockerTopAppBar(
            title = stringResource(id = string.feature_applist_api_app_name),
            actions = {
                if (uiState is AppListUiState.Success) {
                    IconButton(onClick = { showAppSortBottomSheet(true) }) {
                        Icon(
                            imageVector = BlockerIcons.Sort,
                            contentDescription = stringResource(id = string.feature_applist_api_sort_menu),
        ...
        when (uiState) {
            ...
            is AppListUiState.Success -> {
                ...
                Box(modifier = Modifier.pullRefresh(refreshingState)) {
                    if (appList.isEmpty()) {
                        EmptyScreen(textRes = string.feature_applist_api_no_applications_to_display)
                    } else {
                        AppList(
                            appList = appList,
                            highlightSelectedApp = highlightSelectedApp,
...
private fun AppListScreenPreview(
    ...
    BlockerTheme {
        Surface {
            AppListScreen(uiState = AppListUiState.Success(), appList = appList)
        }
    }
}
...
private fun AppListScreenInitialPreview() {
    BlockerTheme {
        Surface {
            AppListScreen(uiState = AppListUiState.Initializing("Blocker"), appList = listOf())
        }
    }
}
...
private fun AppListScreenErrorPreview() {
    BlockerTheme {
        Surface {
            AppListScreen(uiState = AppListUiState.Error(UiMessage("Error")), appList = listOf())
        }
    }
}
...
private fun AppListScreenEmptyPreview() {
    BlockerTheme {
        Surface {
            AppListScreen(uiState = AppListUiState.Success(), appList = listOf())
        }
    }
}
