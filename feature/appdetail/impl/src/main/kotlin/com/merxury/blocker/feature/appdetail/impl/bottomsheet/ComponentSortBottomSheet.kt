...
import com.merxury.blocker.feature.appdetail.impl.bottomsheet.ComponentSortInfoUiState.Loading
...
fun SortOptionsContent(
    ...
    val sortModeList = listOf(
        COMPONENT_NAME to string.feature_appdetail_api_component_name,
        PACKAGE_NAME to string.feature_appdetail_api_package_name,
    )
    val sortByRuleList = listOf(
        ASCENDING to string.feature_appdetail_api_ascending,
        DESCENDING to string.feature_appdetail_api_descending,
    )
    val priorityList = listOf(
        NONE to string.feature_appdetail_api_none,
        DISABLED_COMPONENTS_FIRST to string.feature_appdetail_api_disabled_first,
        ENABLED_COMPONENTS_FIRST to string.feature_appdetail_api_enabled_first,
    )
    Column(modifier = modifier.fillMaxWidth()) {
        Text(
            text = stringResource(id = string.feature_appdetail_api_sort_options),
            style = MaterialTheme.typography.headlineSmall,
            textAlign = TextAlign.Center,
            modifier = Modifier.fillMaxWidth(),
        )
        ItemHeader(title = stringResource(id = string.feature_appdetail_api_sort_by))
        SegmentedButtons(
            items = sortModeList,
            selectedValue = uiState.componentSortInfo.sorting,
            onItemSelection = onSortByClick,
        )
        ItemHeader(title = stringResource(id = string.feature_appdetail_api_order))
        SegmentedButtons(
            items = sortByRuleList,
            selectedValue = uiState.componentSortInfo.order,
            onItemSelection = onSortOrderClick,
        )
        ItemHeader(title = stringResource(id = string.feature_appdetail_api_priority))
        SegmentedButtons(
            items = priorityList,
